...
//Давайте спробуємо вирішити одне й те саме завдання в рамках ООП та ФП. Напишемо модуль, який додаватиме і відніматиме значення.
class Calculator {
    //Зберігаємо (інкапсулюємо) значення всередині модуля у полі value.
    constructor(initial = 0) {
      this.value = initial;
    }
    //При додаванні збільшуємо value на вказану кількість.
    add: (x) => {
      this.value += x;
    }
    //При відніманні зменшуємо value на вказану кількість.
    subtract: (x) => {
      this.value -= x;
    }
  valueOf: () => this.value
